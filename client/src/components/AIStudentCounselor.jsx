  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error('Doubt endpoint failed');
      }
    } catch (err) {
      console.error(err);
      setMessages(prev => [
        ...prev,
        {
          id: Date.now() + 1,
          sender: 'bot',
          text: "I'm sorry, I encountered a communication error with my learning brain. Please make sure the backend services are online."
        }
      ]);
    } finally {
      setLoading(false);
